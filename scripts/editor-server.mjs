  ...
  return {
    ...
    save({ html, revision }) {
      if (typeof html !== 'string' || !html.trim() || html.length > 3_000_000) {
        throw Object.assign(new Error('Nội dung trống hoặc quá lớn; chưa lưu thay đổi.'), { status: 400 });
      }
      const previous = read();
      if (revision !== previous.revision) {
        throw Object.assign(new Error('Tài liệu đã được sửa ở cửa sổ khác. Tải bản nháp trước, rồi mở lại trang để đối chiếu; bản trên đĩa chưa bị ghi đè.'), { status: 409 });
      }
      const prepared = prepareGameplay(html);
      if (!prepared.toc.length) throw Object.assign(new Error('Giữ ít nhất một tiêu đề trong tài liệu.'), { status: 400 });
      ...
      writeFileSync(source + '.pending', prepared.html, 'utf8');
      writeFileSync(output + '.pending', page, 'utf8');
      try {
        renameSync(source + '.pending', source);
        renameSync(output + '.pending', output);
      } catch (error) {
        writeFileSync(source, previous.html, 'utf8');
...
export function createEditorServer({ root = projectRoot, renderPage = html => buildPage(buildDocs(html)) } = {}) {
  ...
  const publicFiles = new Set(['Divergency_Reviewer_Tabs.html', 'steam-image-tool.html', 'rewards-card-poster.html', 'robots.txt', 'sitemap.xml']);
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      ))) return json(res, 403, { error: 'Yêu cầu phải đến từ cửa sổ biên tập trên máy này.' });
      if (pathname === '/api/editor' && req.method === 'GET') {
        return json(res, 200, { app: 'divergency-editor', root, token });
      }
      if (pathname === '/api/images') {
        if (req.headers['x-editor-token'] !== token) return json(res, 403, { error: 'Phiên biên tập hết hạn. Lưu bản nháp rồi mở lại trang.' });
        if (req.method === 'GET') return json(res, 200, { images: listImages(root) });
        if (req.method !== 'POST') return json(res, 405, { error: 'Phương thức không hỗ trợ.' });
        if (Number(req.headers['content-length']) > MAX_IMAGE_BYTES) return json(res, 413, { error: 'Mỗi ảnh cần nhỏ hơn hoặc bằng 20 MB.' });
        const chunks = [];
        let size = 0;
        for await (const chunk of req) {
          size += chunk.length;
          if (size > MAX_IMAGE_BYTES) return json(res, 413, { error: 'Mỗi ảnh cần nhỏ hơn hoặc bằng 20 MB.' });
          chunks.push(chunk);
        }
        let name;
        try { name = decodeURIComponent(req.headers['x-image-name'] || 'anh'); }
        catch { return json(res, 400, { error: 'Tên ảnh không hợp lệ.' }); }
        return json(res, 201, { image: saveImage(root, name, Buffer.concat(chunks)) });
      }
      if (pathname === '/api/gameplay') {
        if (req.headers['x-editor-token'] !== token) return json(res, 403, { error: 'Phiên biên tập hết hạn. Tải bản nháp, rồi mở lại trang.' });
        if (req.method === 'GET') return json(res, 200, store.read());
        if (req.method !== 'PUT') return json(res, 405, { error: 'Phương thức không hỗ trợ.' });
        if (!req.headers['content-type']?.startsWith('application/json')) return json(res, 415, { error: 'Cần nội dung JSON.' });
        let size = 0;
        const chunks = [];
        for await (const chunk of req) {
          size += chunk.length;
          if (size > 6_000_000) return json(res, 413, { error: 'Nội dung quá lớn. Chưa lưu thay đổi.' });
          chunks.push(chunk);
        }
        let body;
        try { body = JSON.parse(Buffer.concat(chunks).toString('utf8')); }
        catch { return json(res, 400, { error: 'Nội dung gửi lên không hợp lệ.' }); }
      ...
      if (!publicFiles.has(relative) && !relative.startsWith('imgs/')) return json(res, 404, { error: 'Không tìm thấy.' });
      ...
      if (relative.startsWith('imgs/') && (!file.startsWith(assetRoot) || !realpathSync(file).startsWith(realpathSync(path.join(root, 'imgs')) + path.sep))) {
        return json(res, 403, { error: 'Đường dẫn không hợp lệ.' });
      }
      if (!statSync(file).isFile()) return json(res, 404, { error: 'Không tìm thấy.' });
      ...
      if (!res.headersSent) json(res, error.status || (error.code === 'ENOENT' ? 404 : 500), {
        error: error.status ? error.message : 'Không đọc hoặc lưu được tài liệu. Giữ bản nháp và kiểm tra chương trình biên tập trên máy.',
      });
      else res.destroy();
    }
  });
